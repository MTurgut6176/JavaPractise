...
*/
       char[][] answers = {
        ...
        for (int i = 0; i < 8; i++) {
            int counter = 0;
            char ogrAnswr[] = answers[i];

            for (int k = 0; k < 10; k++) {
                if (keys[k] == ogrAnswr[k]) {
                    counter++;
                }
            }
   ...
   public static void testsonucu(char[][] answers, char[] keys) {
        ...
        for (int i = 0; i < answers.length; i++) {  // her bir ogrenciyi dolastik i=ogrenci sayisi
            int sayac = 0; // her bir ogrencinin dogru cvp sayisi
            for (int j = 0; j < answers[i].length; j++) { //j cevap anahtari
